...
public class MapPanel extends JPanel implements MouseListener, Observer {
    ...
    private boolean toAssign = false;
    ...
    public MapPanel(SeisEventList se, boolean _toAssign) {
        this.seList = se;

        this.toAssign = _toAssign;
    ...
    // When selectedSiesEventList Changes or TaskBlock selection changed
    @Override
    public void update(Observable o, Object arg) {
        VBASLogger.logDebug("Update map...");
        this.removeAll();
        this.updateUI();
        this.repaint();
    }
    ...
    public void paintComponent(Graphics g) {
        ...
        for (SeisEvent ev : seList.getSeisEventList()) {
            ...
            if (ev.getTSelction() == false && ev.getGSelction() == false && ev.getblAssigned() == toAssign) {
        ...
        for (SeisEvent ev : seList.getSeisEventList()) {
            //Day currDay = new Day(ev.getOrigTime());
            //if(startDay != null && endDay != null && currDay.compareTo(startDay)>=0 && currDay.compareTo(endDay)<=0)
            if (ev.getTSelction() == true && ev.getGSelction() == false && ev.getblAssigned() == toAssign) {
                //px = (int) (dispx + (OsmMercator.LonToX(ev.getLon(), 0)*scale));
                py = (int) (dispy + (OsmMercator.LatToY(ev.getLat(), 0) * scale));
                px = ((OsmMercator.LonToX(ev.getLon(), 0) + transX) > 0) ? (int) ((OsmMercator.LonToX(ev.getLon(), 0) + transX) * scale) : (int) ((OsmMercator.LonToX(ev.getLon(), 0) + transX + 256) * scale);
                px = (int) (dispx + px);
        ...
        for (SeisEvent ev : seList.getSeisEventList()) {
            //Day currDay = new Day(ev.getOrigTime());
            //if(startDay != null && endDay != null && currDay.compareTo(startDay)>=0 && currDay.compareTo(endDay)<=0)
            if (ev.getTSelction() == true && ev.getGSelction() == true && ev.getblAssigned() == toAssign) {
                //px = (int) (dispx + (OsmMercator.LonToX(ev.getLon(), 0)*scale));
                py = (int) (dispy + (OsmMercator.LatToY(ev.getLat(), 0) * scale));
                px = ((OsmMercator.LonToX(ev.getLon(), 0) + transX) > 0) ? (int) ((OsmMercator.LonToX(ev.getLon(), 0) + transX) * scale) : (int) ((OsmMercator.LonToX(ev.getLon(), 0) + transX + 256) * scale);
                px = (int) (dispx + px);
